---
title: "Paracetamol Overdose Summary"
output:
  word_document:
    fig_height: 2
    fig_width: 4
...
#Custom ggplot2 theme for the document
  theme_bw2 <- theme_set(theme_bw(base_size = 6))
````

### Patient and paracetamol information

``` {r Patient and Paracetamol Information, echo=FALSE, warning=FALSE}
#Calculate AGE
...
#Assign label to product category ingested
  if (input$PROD == 1) PROD <- "Paracetamol alone"
  if (input$PROD == 2) PROD <- "Paracetamol and antihistamine"
  if (input$PROD == 3) PROD <- "Paracetamol and opioid"
  if (input$PROD == 4) PROD <- "Paracetamol and other"
  if (input$PROD == 5) PROD <- "Extended-release Paracetamol"
...
#Assign labels to PAC and TIME
	PAC1 <- input$PAC1  #First plasma paracetamol concentration (mg/L)
	TIME1 <- input$TIME1  #TIme for first plasma paracetamol concentration (hours)
	...
	if (input$NPAC > 1) {
		PAC2 <- input$PAC2  #Second plasma paracetamol concentration (mg/L)
		TIME2 <- input$TIME2  #Time for second plasma paracetamol concentration (hours)
	}
```
...
### Sampled plasma paracetamol concentrations (PAC)
...
### Individual paracetamol concentration-time profile

``` {r Individual paracetamol concentration-time profile, echo=FALSE, warning=FALSE}
